...
for i in range(10):
    ...
    image = paramgraphics.mat_to_img(x_gen_batch.T, dim_input,colorImg=colorImg,tile_shape=(num_classes,num_classes),scale=generation_scale,save_path=os.path.join(results_dir,'interpolation-'+str(i)+'.png'))
...
for i in range(10):
    ...
    x_gen_batch = x_gen_batch.reshape(batch_size_g,-1)
    image = paramgraphics.mat_to_img(x_gen_batch.T, dim_input, colorImg = colorImg, tile_shape=(num_classes,num_classes), scale=generation_scale, save_path=os.path.join(results_dir,'random-' + str(i) + '.png'))

for i in range(10):
    sample_y = np.int32(np.repeat(np.arange(num_classes), batch_size_g/num_classes))
    x_gen_batch = generate_shared(sample_y)
    x_gen_batch = x_gen_batch.reshape(batch_size_g, -1)
    image = paramgraphics.mat_to_img(x_gen_batch.T, dim_input, colorImg = colorImg, tile_shape=(num_classes,num_classes), scale=generation_scale, save_path=os.path.join(results_dir,'shared-' + str(i) + '.png'))
