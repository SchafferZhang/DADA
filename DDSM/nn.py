...
def adam_updates(params, cost, lr=0.001, mom1=0.9, mom2=0.999):
    ...
    for p, g in zip(params, grads):
        v = th.shared(np.cast[th.config.floatX](p.get_value() * 0.), broadcastable=p.broadcastable)
        mg = th.shared(np.cast[th.config.floatX](p.get_value() * 0.), broadcastable=p.broadcastable)
        v_t = mom1 * v + (1. - mom1) * g
...
class MeanOnlyBNLayer(lasagne.layers.Layer):
    def __init__(self, incoming, b=lasagne.init.Constant(0.), g=lasagne.init.Constant(1.),
                 W=lasagne.init.Normal(0.05), nonlinearity=relu, modify_incoming=True, momentum=0.999, **kwargs):
